...
- **For the risky tier, separate the verifiable fact from the speculative cause.** (Superseded by the 2026-07-10 amendment: implemented as draft-time prevention instead.) A chart-movement claim ("re-entered the top ten, a new peak") is verifiable from the pipeline's own current-versus-carried rank data and needs no external source. A causal claim ("after a film placement") is speculative and publishes only when an authoritative source states it; absent that, the causal clause is dropped while the movement fact stands.
...
## Amendment (2026-07-10): the risky tier is guarded at draft time, not by clause-splitting

The fact-versus-cause mechanism above was never built; the shipped guard reaches the same end one stage earlier. The Decision bullet specified route-time repair: split a risky blurb into a movement fact, verified against the pipeline's own rank data, and a causal clause that publishes only when a source states it. In the code, the router drops whole entries only (`routeStore` in `scripts/commentary/route.ts`), no clause-level machinery exists, and no check verifies movement claims against rank data.

What replaced it is prevention in the drafting prompt (`src/lib/drafting.ts`). The prompt forbids asserting chart positions in the lead or detail at all: the pipeline's rank data is handed to the drafter as selection context only, because no outlet publishes our rankings, so grounding could never verify a claim built on them. The timely hook lives in the `tag`, which grounding does not check and the card renders as a label rather than a claim. A causal "why it is rising" claim is allowed only when a source explicitly states that cause, which the per-entry grounding check then enforces.

The trade: clause-splitting would have salvaged partial cards (movement fact kept, cause dropped), while the draft-time rule produces cards that carry no movement assertions to begin with, so an entry publishes intact or drops intact. That matches the gate's shape (binary, entry-level, fail-closed) and costs only the movement-fact sentence the original mechanism meant to keep. The idea of a rank-data-verified movement fact stays available as future work; nothing in the gate blocks adding it.
